...
def train_test(num_rows=None):
    ...
    train_df.loc[train_df['target'] < -15, 'outliers'] = 1
    train_df.loc[train_df['target'] > 15, 'outliers'] = 1
...
def kfold_lightgbm(train_df, test_df, target, num_folds, stratified=False, debug=False):
    ...
    print('Final RMSE : %.6f' % (rmse(target, oof_preds)))
...
def main(debug=False):
    ...
    with timer("Run LightGBM with kfold"):
        kfold_lightgbm(train_df, test_df, target, num_folds=11, stratified=False, debug=debug)
